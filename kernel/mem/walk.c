    ...
    uintptr_t end, struct page_walker *walker)
{
	...
	uintptr_t page,page_end;
    physaddr_t *entry;
    int ret = 0;

    for(page = page_end = base; page_end < end; page = sign_extend(page_end+1)){
        page_end = ptbl_end(page);
       
        entry= ptbl->entries + PAGE_TABLE_INDEX(page);

        if(walker->get_pte){
            if( (ret = walker->get_pte(entry,page,page_end < end ? page_end : end,walker)) < 0) return ret;
        }

        if(walker->pte_hole && !(PAGE_PRESENT & *entry) ){
            if( (ret = walker->pte_hole(page,page_end < end ? page_end : end,walker)) < 0 ) return ret;
        }
    ...
    uintptr_t end, struct page_walker *walker)
{
    ...
    for(page = page_end = base; page_end < end; page = sign_extend(page_end+1)){ //sign extend 
        page_end = pdir_end(page);
        entry = pdir->entries + PAGE_DIR_INDEX(page);
        
        if(walker->get_pde){
            if( (ret = walker->get_pde(entry,page,page_end,walker)) < 0) return ret;
        }

        if(walker->pte_hole && !(PAGE_PRESENT & *entry) ){
            if( (ret = walker->pte_hole(page,page_end,walker)) < 0 ) return ret;
        }

        if((PAGE_PRESENT & *entry)){
            ptbl = (struct page_table*)KADDR(PAGE_ADDR(pdir->entries[PAGE_DIR_INDEX(page)]));
            ret = ptbl_walk_range(ptbl,page,page_end < end ? page_end : end,walker);
            
            if(walker->unmap_pde){
                if( (ret = walker->unmap_pde(entry,page,page_end < end ? page_end : end,walker)) < 0) return ret;
            }
        }
	}
	return ret;
    ...
    uintptr_t end, struct page_walker *walker)
{
    ...
    for(page = page_end = base; page_end < end; page = sign_extend(page_end+1)){ //sign extend 
        page_end = pdpt_end(page);

        entry= pdpt->entries + PDPT_INDEX(page);

        if(walker->get_pdpte){
            if( (ret = walker->get_pdpte(entry,page,page_end,walker)) < 0) return ret;
        }
        ...
        if((PAGE_PRESENT & *entry)){
            ...
            ret = pdir_walk_range(pdir,page,page_end < end ? page_end : end,walker);

            if(walker->unmap_pdpte){
                if( (ret = walker->unmap_pdpte(entry,page,page_end,walker)) < 0) return ret;
            }
    ...
    struct page_walker *walker)
{
    ...
    for(page = page_end = base; page_end < end; page = sign_extend(page_end+1)){ //sign extend 
        page_end = pml4_end(page);

        entry= pml4->entries + PML4_INDEX(page);

        if(walker->get_pml4e){
            if( (ret = walker->get_pml4e(entry,page,page_end,walker)) < 0) return ret;
        }
        ...
        if((PAGE_PRESENT & *entry)){
            ...
            ret = pdpt_walk_range(pdpt,page,page_end < end ? page_end : end,walker);
            
            if(walker->unmap_pml4e){
                if( (ret = walker->unmap_pml4e(entry,page,page_end,walker)) < 0) return ret;
            }
        }
	}
