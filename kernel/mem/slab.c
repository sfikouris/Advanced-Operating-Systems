...
{
	/* LAB 3: your code here. */
	void *base = (void*)((uintptr_t)info - slab->info_off);
	list_remove(&info->node);
	page_free(pa2page(PADDR(base)));
	// page_lookup(kernel_pml4,PADDR(base))
}
