	...
	uintptr_t offset)
{
	...
	if ((uintptr_t)addr + len > USER_LIM) return MAP_FAILED;

	if (flags & MAP_FIXED) {
		if (remove_vma_range(cur_task, addr, len) < 0) return MAP_FAILED;
	}

	if( ((prot & PROT_READ) != PROT_READ) && prot) return MAP_FAILED;

	if( (prot & ( PROT_WRITE | PROT_EXEC)) == (PROT_WRITE | PROT_EXEC) ) return MAP_FAILED;
	
	if( (flags & ( MAP_ANONYMOUS | MAP_PRIVATE)) != (MAP_ANONYMOUS | MAP_PRIVATE) ) return MAP_FAILED;

	vma = add_vma(cur_task,"user",addr,len,prot);
	if(!vma) return MAP_FAILED;

	if (flags & MAP_POPULATE) {
		if (populate_vma_range(cur_task, addr, len, prot) < 0) return MAP_FAILED;
	}
