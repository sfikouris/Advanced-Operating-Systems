...
struct gdt_entry gdt_entries[5 + NCPUS * 2] = {
...
{
	/* LAB 6: your code here. */
	size_t cpu = lapic_cpunum();
	this_cpu->cpu_tss.rsp[0] = KSTACK_TOP - cpu*(PAGE_SIZE + PAGE_SIZE);

	set_tss_entry((struct tss_entry *)(gdt_entries + (GDT_TSS0 >> 3) + (cpu * 2)),
	    &this_cpu->cpu_tss);
	load_gdt(&gdtr, GDT_KCODE, GDT_KDATA);
	load_task_sel(GDT_TSS0 + ((cpu*2) << 3));
}
